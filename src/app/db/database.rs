        ...
        pub async fn update_person(uuid:String, title:String, level: String, compensation: i32) -> Result<Option<Person>, PersonError> {
            ...
            match find_person {
                Ok(found) => {
                    match found {
                        Some(found_person) => {
                            let updated_user: Result<Option<Person>, Error> = DB.update(("person", &uuid))
                                .merge(Person::new(uuid, found_person.name, title, level, compensation, found_person.joined_date))
                                .await;
                            let _ = DB.invalidate().await;
